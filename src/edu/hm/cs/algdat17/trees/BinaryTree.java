...
/**
 * Class representing a binary tree storing only key values (like sets). This
 * class does not provide any operations changing the tree, as any dynamic
 * operations have to be provided by the subclasses adding additional
 * restrictions.
 * 
 * @author katz.bastian
 *
...
public class BinaryTree<T> {
	...
	private int getLeafeCount(TreeNode<T> node) {
		if (node.left == null && node.right == null) {
			return 1;
		}

		if (node.left != null && node.right != null) {
			return getLeafeCount(node.left) + getLeafeCount(node.right);
		} else {
			if (node.left != null) {
				return getLeafeCount(node.left);
			} else {
				return getLeafeCount(node.right);
			}
		}
	}
	...
	private boolean contains(T elemet, TreeNode<T> node) {
		if (node.data.equals(elemet)) {
			return true;
		}
		...
		if (node.left != null && node.right != null) {
			if (contains(elemet, node.left)) {
				return true;
			} else {
				return contains(elemet, node.right);
			}
		} else {
			if (node.left != null) {
				return contains(elemet, node.left);
			} else {
				return contains(elemet, node.right);
			}
		}
	...
	/**
	 * Helper method. Recursively calculates the number of nodes in the
	 * (sub)tree.
	 * 
	 * @param node
	 *            tree node
	...
	/**
	 * Calculates the height of the tree. By definition, the empty tree has
	 * height 0, a tree with one node has height 1.
	 * 
	...
	/**
	 * Applies an operation on all nodes in the tree in pre-, in- or post-order,
	 * i.e. in any order produces by depth-first-search.
	 * 
	 * @param operation
	 *            operation to be applied to all nodes
	 * @param order
	 *            defines in which order the operation is applied to the node
	 *            and the subtrees
	 */
	...
	/**
	 * Helper method to recursively apply an operation on all nodes in the tree
	 * in pre-, in- or post-order, i.e. in any order produces by
	 * depth-first-search.
	 * 
	 * @param node
	 *            tree node
	 * @param operation
	 *            operation to be applied to all nodes in the subtree of the
	 *            given node
	 * @param order
	 *            defines in which order the operation is applied to the node
	 *            and the subtrees
